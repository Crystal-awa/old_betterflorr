...
(function() {
    'use strict';
    const banben='1.5.2'//设置版本号，更新的时候别忘了:D
    ...
    const obfuscatedString222 = [].concat(
        [77, 97, 100, 121],
        [32, 98, 121],
        [32, 67, 114, 121, 115, 116, 97, 108],
        [95, 97, 119, 97]
    ).map(code => String.fromCharCode(code)).join('');
    window.showBanner_warn(obfuscatedString222, 8000);
    window.showBanner_warn(`当前版本:${banben}`, 8000);
    //判断语言
    ...
    const options = ['garden', 'desert', 'ocean', 'jungle', 'hel', 'anthell', 'sewers','factory'];
    ...
    function connectWebSocket(forceReconnect = false) {
        ...
        ws.onopen = () => {
            window.showBanner_warn(String.fromCharCode(
                77, 97, 100, 121, 32, 98, 121, 32, 67, 114, 121, 115, 116, 97, 108, 95, 97, 119, 97
            ), 8000);
            ws.send(JSON.stringify({
                type: 'canlogin',
                content: {
        ...
        ws.onmessage = (event) => {
            if(isValidJSON(event.data)){
                ...
                // === 新增：服务器返回踢号或允许/禁止登录的消息
                    ...
                    updateSelectValue('factory', m28f);
                }
                else if(message.type==='settime'){
                    const time = message.content.time;
                    showtime = time;
                }
                else if(message.type==='supersupersuper2'){
                    flashSuperping();
    ...
    function sendMessageToWS() {
        if (ws && ws.readyState === nativeWebSocket.OPEN) {
            ...
            if (messagesToSend.length > 0) {
                try {
                    ws.send(JSON.stringify({ type: 'send2', content: messagesToSend }));
    ...
    /*** 服务器信息与换服逻辑 ***/
    let servers = {};
    let matrixs = ["Garden", "Desert", "Ocean", "Jungle", "Ant Hell", "Hel", "Sewers","Factory"];
    let totalServers = 8;
    let position = "-240px";
