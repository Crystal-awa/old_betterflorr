...
(function() {
    ...
    superping.style.position = 'fixed';
    superping.style.top = '10px';
    superping.style.left = '10px';
    superping.style.width = '200px';
    superping.style.maxHeight = '180px';  // 三行文字，每行15px
    superping.style.height = '180px'
    superping.style.overflowY = 'auto';
    superping.style.backgroundColor = 'rgba(255, 255, 255, 0.8)';  // 白色半透明背景
    superping.style.padding = '10px';
    superping.style.borderRadius = '10px';
    superping.style.boxShadow = '0px 0px 10px rgba(0, 0, 0, 0.2)';
    superping.style.fontSize = '14px';
    superping.style.lineHeight = '25px';
    //superping.textContent = 'super播报'//不需要了
    superping.style.overflowY = 'auto';   // 添加垂直滚动条
    // 创建一个红绿点
    // 在 superping 容器右上角添加红绿点
    const statusDot = document.createElement('div');
    statusDot.style.position = 'absolute';
    statusDot.style.top = '2px';  // 从右上角往上偏移 5px
    statusDot.style.right = '2px'; // 从右上角往右偏移 5px
    statusDot.style.width = '12px'; // 圆点的宽度
    statusDot.style.height = '12px'; // 圆点的高度
    statusDot.style.borderRadius = '50%';  // 圆形
    statusDot.style.backgroundColor = 'red'; // 默认是红色（未连接）

    // 将圆点添加到 superping 容器中
    



    const title = document.createElement('div');
    title.textContent = ''; // 清空默认的文本内容
    title.innerHTML = '<span style="color: gold; font-size: 14px; font-weight: bold; text-shadow: 2px 2px 5px rgb(0, 0, 0);">super播报</span>';  // 使用 <span> 来设置颜色
    title.style.position = 'sticky';  // 使用 sticky 保持标题始终在顶部
    title.style.top = '0';  // 确保标题固定在顶部
    title.style.marginBottom = '10px';  // 调整标题与内容之间的间距
    superping.appendChild(title);  // 将标题添加到 superping 容器中
    ...
    if (savedPosition) {
        ...
        localStorage.setItem('superpingPosition', JSON.stringify(position));
    }
    // 鼠标按下事件：开始拖动
    superping.addEventListener('mousedown', (e) => {
        ...
        superping.style.transition = 'none'; // 禁用过渡效果，保证拖动时流畅
    });
    ...
    document.addEventListener('mousemove', (e) => {
        if (isDragging) {
            ...
            superping.style.left = `${x}px`;
            superping.style.top = `${y}px`;
        }
    });
    ...
    document.addEventListener('mouseup', () => {
        ...
        isDragging = false;
        superping.style.transition = 'all 0.3s ease'; // 恢复过渡效果
    });
    ...
    function displayText(text) {
        ...
        // 设置文本元素的样式（如果需要）
        textElement.style.marginBottom = '10px';  // 设置每条文本之间的间距
        textElement.style.fontSize = '14px';      // 字体大小
        textElement.style.lineHeight = '20px';    // 行高
    
        // 将文本元素添加到 content 容器
        const content = superping.querySelector('div');  // 获取 content 容器
    ...
    window.onresize = function() {
        ...
        if(isSuperpingEnabled){
            ...
            // 获取存储的比例并根据窗口尺寸重新计算位置
            const savedPosition = JSON.parse(localStorage.getItem('superpingPosition'));
            if (savedPosition) {
                const left = savedPosition.left * width;
                const top = savedPosition.top * height;
                superping.style.left = `${left}px`;
                superping.style.top = `${top}px`;
            }
        }
    ...
    function updateSuperpingPosition() {
        const isSuperpingEnabled = getSwitchState(1);  // 假设第二个开关的状态是 getSwitchState(2)
   ...
   updateSuperpingPosition();
    ...
    function connectWebSocket() {
        ...
        ws.onopen = () => {
            ...
            statusDot.style.backgroundColor ='rgb(19, 240, 144)';  // 设置为绿色
            if (reconnectTimer) {
        ...
        ws.onerror = (error) => {
            ...
            statusDot.style.backgroundColor = 'yellow';  // 设置为黄色
        };

        ws.onclose = () => {
            showMessage('[WebSocket] 连接已关闭，10秒后尝试重连...');
            console.warn('[WebSocket] 连接已关闭，10秒后尝试重连...');
            statusDot.style.backgroundColor = 'red';  // 设置为红色
        ...
        ws.onmessage = (event) => {
            showMessage('[WebSocket] 收到消息:', event.data);
            // 调用 displayText 函数将消息显示到界面
